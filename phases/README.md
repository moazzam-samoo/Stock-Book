...
| 09 — Searchable ticker picker | [PHASE-09-ticker-search.md](PHASE-09-ticker-search.md) | 08's new Task 7 | Ready — new phase added 2026-09-05. Every ticker-entry field (Favorites, transactions, Phase 07's buy alerts) is free-text today; `TickerAutocomplete` only suggests from the user's own Favorites, which is empty of exactly the tickers a buy alert targets. Widens it to search the full PSX ticker universe by symbol or company name, cached locally for offline/instant search |
